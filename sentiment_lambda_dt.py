...
#tf_train = sc.textFile("hdfs://VM10-1-0-14:9000/classifier/tf").map(Vectors.parse)
#idf_train = IDF().fit(tf_train)
#tf_test = HashingTF(numFeatures=numFeatures).transform(prep_filtered.map(porter_stem, preservesPartitioning=True))
#tfidf_test = idf_train.transform(tf_test)
#idf_test = IDF().fit(tf_test)
#tfidf_test = idf_test.transform(tf_test)

#labeled_prediction = review_labels.zip(model.predict(train_tfidf)).map(lambda x: {"actual": x[0], "predicted": x[1]})
#accuracy = 100 * labeled_prediction.filter(lambda doc: doc["actual"] == doc['predicted']).count() / labeled_prediction.count()
#print(accuracy)
